---
{
  "title": "RSI \u2014 EURUSD Backtest Results (5-Year)",
  "date": "2026-09-12",
  "draft": false,
  "type": "backtests",
  "period": "2021-09-13 \u2192 2026-09-11",
  "metrics": [
    {
      "label": "Total Return",
      "value": "-11.3%"
    },
    {
      "label": "CAGR",
      "value": "-2.4%"
    },
    {
      "label": "Sharpe Ratio",
      "value": "-0.53"
    },
    {
      "label": "Max Drawdown",
      "value": "20.9%"
    },
    {
      "label": "Win Rate",
      "value": "20.0%"
    },
    {
      "label": "Profit Factor",
      "value": "0.56"
    },
    {
      "label": "Total Trades",
      "value": "20"
    },
    {
      "label": "Avg Trade",
      "value": "86 days"
    }
  ],
  "yearly": [
    {
      "year": "2021",
      "return_pct": "-4.1%",
      "trades": "3",
      "win_rate_pct": "20.0%"
    },
    {
      "year": "2022",
      "return_pct": "-6.3%",
      "trades": "5",
      "win_rate_pct": "20.0%"
    },
    {
      "year": "2023",
      "return_pct": "+3.3%",
      "trades": "2",
      "win_rate_pct": "20.0%"
    },
    {
      "year": "2024",
      "return_pct": "-5.9%",
      "trades": "6",
      "win_rate_pct": "20.0%"
    },
    {
      "year": "2025",
      "return_pct": "+13.5%",
      "trades": "1",
      "win_rate_pct": "20.0%"
    },
    {
      "year": "2026",
      "return_pct": "-1.3%",
      "trades": "3",
      "win_rate_pct": "20.0%"
    }
  ],
  "cta_headline": "Trade Forex with TradingView Pro",
  "cta_body": "Execute directly from charts with integrated broker connections and real-time data.",
  "cta_label": "Get TradingView Pro",
  "cta_url": "https://www.tradingview.com/?aff_id=166324",
  "equity_curve_url": "/backtests/rsi-oversold-overbought-eurusd-equity.png",
  "crosslinks": [
    {
      "title": "halftrend_long_short_signal_engine review: settings, strategy & how to use it",
      "url": "/reviews/halftrend-long-short-signal-engine/"
    },
    {
      "title": "volume_ma review: settings, strategy & how to use it",
      "url": "/reviews/volume-ma/"
    },
    {
      "title": "rsi macd combo signal review: settings, strategy & how to use it",
      "url": "/reviews/rsi-macd-combo-signal/"
    }
  ]
}
---

![RSI Oversold/Overbought — EURUSD=X Equity Curve](/backtests/rsi-oversold-overbought-eurusd-equity.png)

## How the RSI Oversold/Overbought Works

The rsi oversold/overbought is one of the most widely-used signals in trading. It's simple, mechanical, and removes emotional decision-making from your trading by generating clear entry and exit signals based on the RSI indicator.

On EURUSD, the RSI acts as a trend filter. It won't catch every exact bottom, but it reliably captures the meat of major moves while keeping you out of chop.

## 5-Year Performance on EURUSD

Over five years (2021-09-13 → 2026-09-11), the RSI Oversold/Overbought delivered a **-11.3% total return** on EURUSD=X, compounding at **-2.4% annually**. 

At **-0.53**, the RSI earned less than one units of return for every unit of risk taken. For context, anything above 1.0 is considered good; above 1.5 is excellent.

The catch? A **20.9% max drawdown**. At some point during this 5-year run, you'd have been down nearly 21% from peak equity. That's significant territory for most traders. If you can't handle that kind of drawdown, this isn't for you — or you need to size down.

## Risk Assessment

| Metric | Value | Verdict |
|--------|-------|---------|
| Sharpe Ratio | -0.53 | ❌ Weak |
| Max Drawdown | 20.9% | ⚠️ Moderate |
| Win Rate | 20.0% | ❌ Low — relies on outsized wins |
| Profit Factor | 0.56 | ❌ Unprofitable |
| Trades/Year | ~20 total | Selective — not overtrading |

The **profit factor of 0.56** tells the real story: when the RSI fires, the winning signals don't outweigh the losers. Combined with a 20.0% win rate, this requires careful position sizing to be profitable.

## Does the RSI Oversold/Overbought Work on EURUSD?

The rsi oversold/overbought struggled to generate meaningful edge on EURUSD over this 5-year period. With a **-0.53 Sharpe ratio** and **0.56 profit factor**, the risk-adjusted returns are below what most systematic traders would consider acceptable.

This doesn't mean the RSI is useless — it may work better on different assets, timeframes, or when combined with other filters. But on EURUSD with default parameters over 5 years, it didn't produce a compelling standalone edge.

Our take: skip it on EURUSD unless you're combining it with other confirmations or using it on different timeframes.




---

**Trade Forex with TradingView Pro**

Execute directly from charts with integrated broker connections and real-time data.

👉 [Get TradingView Pro](https://www.tradingview.com/?aff_id=166324)

*The Indicator Lab is supported by readers. Some links are affiliate links — we earn a commission at no extra cost to you.*


<small>Data source: Yahoo Finance (yfinance). Backtest engine: backtrader. 95% position sizing. 0.1% commission per trade. Last refreshed: September 12, 2026. Not financial advice. Past performance does not guarantee future results.</small>

![RSI Oversold/Overbought — Trade Signals on EURUSD=X](/backtests/rsi-oversold-overbought-eurusd-trades.png)
