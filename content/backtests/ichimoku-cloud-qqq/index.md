---
{
  "title": "Ichimoku \u2014 QQQ Backtest Results (5-Year)",
  "date": "2026-05-25",
  "draft": false,
  "type": "backtests",
  "period": "2021-05-26 \u2192 2026-05-22",
  "metrics": [
    {
      "label": "Total Return",
      "value": "+64.6%"
    },
    {
      "label": "CAGR",
      "value": "10.5%"
    },
    {
      "label": "Sharpe Ratio",
      "value": "0.54"
    },
    {
      "label": "Max Drawdown",
      "value": "22.4%"
    },
    {
      "label": "Win Rate",
      "value": "42.1%"
    },
    {
      "label": "Profit Factor",
      "value": "2.17"
    },
    {
      "label": "Total Trades",
      "value": "19"
    },
    {
      "label": "Avg Trade",
      "value": "59 days"
    }
  ],
  "yearly": [
    {
      "year": "2021",
      "return_pct": "+19.5%",
      "trades": "1",
      "win_rate_pct": "42.1%"
    },
    {
      "year": "2022",
      "return_pct": "-33.2%",
      "trades": "5",
      "win_rate_pct": "42.1%"
    },
    {
      "year": "2023",
      "return_pct": "+55.9%",
      "trades": "5",
      "win_rate_pct": "42.1%"
    },
    {
      "year": "2024",
      "return_pct": "+27.7%",
      "trades": "3",
      "win_rate_pct": "42.1%"
    },
    {
      "year": "2025",
      "return_pct": "+21.0%",
      "trades": "4",
      "win_rate_pct": "42.1%"
    },
    {
      "year": "2026",
      "return_pct": "+17.2%",
      "trades": "1",
      "win_rate_pct": "42.1%"
    }
  ],
  "cta_headline": "Trade Stocks on Moomoo \u2014 Commission-Free",
  "cta_body": "Access US markets with zero commissions and professional-grade charting tools.",
  "cta_label": "Open Moomoo Account",
  "cta_url": "https://j.moomoo.com/0BpZwx",
  "equity_curve_url": "/backtests/ichimoku-cloud-qqq-equity.png",
  "crosslinks": [
    {
      "title": "ema crossover \u2014 review",
      "url": "/reviews/ema-crossover/"
    },
    {
      "title": "fibonacci retracement \u2014 review",
      "url": "/reviews/fibonacci-retracement/"
    },
    {
      "title": "ichimoku cloud \u2014 review",
      "url": "/reviews/ichimoku-cloud/"
    }
  ]
}
---

![Ichimoku Cloud — QQQ Equity Curve](/backtests/ichimoku-cloud-qqq-equity.png)

## How the Ichimoku Cloud Works

The ichimoku cloud is one of the most widely-used signals in trading. It's simple, mechanical, and removes emotional decision-making from your trading by generating clear entry and exit signals based on the Ichimoku indicator.

On QQQ, the Ichimoku acts as a trend filter. It won't catch every exact bottom, but it reliably captures the meat of major moves while keeping you out of chop.

## 5-Year Performance on QQQ

Over five years (2021-05-26 → 2026-05-22), the Ichimoku Cloud delivered a **+64.6% total return** on QQQ, compounding at **10.5% annually**. 

At **0.54**, the Ichimoku earned less than one units of return for every unit of risk taken. For context, anything above 1.0 is considered good; above 1.5 is excellent.

The catch? A **22.4% max drawdown**. At some point during this 5-year run, you'd have been down nearly 22% from peak equity. That's significant territory for most traders. If you can't handle that kind of drawdown, this isn't for you — or you need to size down.

## Risk Assessment

| Metric | Value | Verdict |
|--------|-------|---------|
| Sharpe Ratio | 0.54 | ⚠️ Moderate |
| Max Drawdown | 22.4% | ⚠️ Moderate |
| Win Rate | 42.1% | ⚠️ Below average |
| Profit Factor | 2.17 | ✅ Excellent |
| Trades/Year | ~19 total | Selective — not overtrading |

The **profit factor of 2.17** tells the real story: when the Ichimoku fires, the winning signals are larger than the losing ones. Combined with a 42.1% win rate, this requires careful position sizing to be profitable.

## Does the Ichimoku Cloud Work on QQQ?

The ichimoku cloud shows moderate edge on QQQ — **0.54 Sharpe, 2.17 profit factor** over 19 trades. It's not a home run, but it's also not random.

The 42.1% win rate means you'll be wrong more than half the time. The profit factor above 1.0 means your wins are bigger than your losses, which is what keeps you profitable.

Our take: usable as a confirmation tool, but not as a standalone system on QQQ. Combine with trend filters or volume confirmation for better results.

<small>Data source: Yahoo Finance (yfinance). Backtest engine: backtrader. Commission: 0.1% modeled. This is not financial advice. Past performance does not guarantee future results.</small>

![Ichimoku Cloud — Trade Signals on QQQ](/backtests/ichimoku-cloud-qqq-trades.png)
