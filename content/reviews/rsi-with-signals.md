...
The default RSI length and the standard overbought/oversold levels are the starting point. On lower timeframes, the defaults are more sensitive; raising the RSI length smooths out the noise. The overbought/oversold thresholds can be widened for strongly trending assets, which produces fewer but more selective signals.

The trend filter setting is where this indicator shines or fails depending on how you configure it. A faster filter setting produces more signals that are less reliable. A slower filter setting means waiting longer for entries, but signal quality improves. For swing trading, the slower setting is generally the better fit. For day trading, a faster filter paired with a longer RSI length tends to suit the pace.

**How It's Used In Practice**

The chart above shows the indicator in action, and you can see how the signals line up with actual momentum shifts rather than just price touching a line. A straightforward entry logic: wait for a bullish signal arrow in an uptrend (price above the trend filter), confirm with RSI crossing back above the 50 midline, then enter on the next candle open. For exits, close half the position when RSI hits the upper threshold and trail the rest with a moving average.

The short side works the same way in reverse, though short signals tend to be less reliable than longs. That's not necessarily the indicator's fault; it's just how RSI behaves in different market conditions. Long-biased traders won't be bothered by it.
...
For traders who need trend context built into their RSI signals, Rsi_With_Signals delivers solid value.

### Does this indicator repaint?

Yes — the arrows can change on the current candle. Wait for the close to confirm. Note that the source material is inconsistent on this point, with one section claiming signals are calculated on closed bars and will not change; the more detailed discussion of the indicator's behavior states the arrows can appear, disappear, or shift on the current candle before confirming. Treat the repainting risk as real and wait for the bar to close.

### Can it be used for crypto?

Yes — it has been used on BTC and ETH, and it works well on higher intraday timeframes.

### What's the best timeframe?

The 1-hour is the practical minimum. Anything lower generates too much noise.

### Does it work for options trading?

Yes, but the slower trend filter helps avoid premium decay from false signals.
