...
- **Trend filter**: Uses a moving average to define the primary direction. Only counter-trend moves against that average are considered pullbacks.
- **Exhaustion detection**: Looks for wick-to-body ratio or RSI divergence within the pullback candle to confirm the sniper shot.
- **Custom alert system**: You can set alerts for when a new sniper signal prints — useful if you can't stare at charts all day.
- **Multi-timeframe aware**: The indicator is intended for higher timeframes. On lower timeframes, it tends to generate more false signals.

## Settings and How to Tune Them

- **Trend EMA period**: The default setting is fine for most pairs; a shorter period may suit faster trends like NAS100.
- **Pullback depth**: A Fibonacci retracement of the last swing, set manually in the settings if available.
- **Min pullback candles**: A minimum number of candles is required to filter out noise.
- **Stop-loss**: The default stop is described as too tight for crypto; a wider ATR-based stop is the commonly suggested adjustment.

**Note**: On the settings panel, consider toggling off "Show All Signals" unless you want a cluttered chart. Keeping only "Confirmed Signals" visible is a common preference.

## How to Use It for Entries and Exits

**Entry logic**:
1. Wait for price to close above the trend EMA (long bias) or below it (short bias).
2. Look for a pullback that retraces a meaningful portion of the prior move.
...
- Trail stop after price moves in your favor by a multiple of ATR.

The indicator's default take-profit is often described as conservative.
...
**Q: Does it repaint?**
A: Signals are fixed once the candle closes. However, the pullback zone may shift if you change settings.
