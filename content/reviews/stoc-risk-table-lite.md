...
You can set the stochastic calculation period (default 14, but adjustable) and choose whether the table uses the current chart timeframe or a higher one. Running a higher-timeframe stochastic alongside a lower-timeframe chart can give better context for intraday trades.

**4. Lightweight performance.**
No heavy calculations. This indicator runs smoothly even on low-end machines or with dozens of charts open.

---

## Settings and How to Tune Them

- **Stochastic length:** The default is 14. Shorter lengths make the reading more reactive; longer lengths smooth it out. Adjust to match your trading horizon.
- **Overbought/oversold levels:** Default 80/20. On volatile pairs, widening these thresholds reduces how often the reading sits in an extreme zone.
- **Table position:** Upper-left is default. If you use other indicators there, move it to another corner via the settings.
- **Smoothing:** The Lite version doesn't include smoothing (that's the Pro version). The raw K-line is more sensitive as a result.

A common configuration is the default length with default levels, applied to a higher timeframe than the chart you're trading.
...
**Q: Does it repaint?**
A: The stochastic K-line is calculated on the current bar, so it updates in real-time but doesn't change past values.
