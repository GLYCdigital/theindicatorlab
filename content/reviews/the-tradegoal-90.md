...
The_Tradegoal_90 is positioned as a momentum and volatility tool for scalping and intraday work. This review covers what it plots, how the signal logic is described, how to tune it, and where it falls short.
...
- **No repaint.** Once an arrow appears, it stays. That's rare for a momentum-based tool.
...
The indicator ships with defaults, and the parameters map to the three plotted components:

- **Momentum Period.** Controls how far back the oscillator looks. Lower values make the oscillator react sooner; higher values smooth it out. The default sits in the mid-teens, and raising it is the usual move for slower timeframes such as 1-hour or 4-hour charts.
- **Volatility Multiplier.** Sets how wide the envelope sits around price. A tighter multiplier reduces noise on fast timeframes; a wider one gives signals more room before they trigger.
- **Threshold Sensitivity.** Determines how much momentum is required before a signal fires. Raising it produces fewer signals; lowering it produces more.
- **Arrow Filter.** When enabled, suppresses arrows while the volatility band is contracting. This is the mechanism behind the indicator's ranging-market behavior.

These settings interact: tightening the volatility multiplier while raising the threshold shifts the tool toward fewer, more selective signals, while loosening both pushes it toward frequency.
...
3. Enter on the next candle's open if price is within a small margin of the band's upper edge.

**Exit:**
- Take profit at the opposite band edge (or a multiple of the band width).
- Stop loss at the band's lower edge or one ATR below entry, whichever is wider.

**Exit (Short):** Reverse the logic.

The indicator tends to behave better when the secondary confirmation line slopes in the same direction as the arrow. Flat or opposing slopes produce more false signals.
