...
The indicator plots an 8 EMA and a 20 EMA, then uses their crossovers as the core signal. On top of that, it adds a momentum filter and a volatility envelope around those averages. The result is three distinct states — strong uptrend, strong downtrend, and chop — each color-coded on the chart. When price is riding the 8 EMA above the 20 EMA with the envelope expanding, you're in a "confirmed" trend. When those lines start weaving through each other, it paints the background a neutral color and essentially tells you to stand down.
...
The momentum confirmation is the differentiator. It's not just "price above the 20" — it checks whether the slope of the 20 EMA itself is accelerating. That subtle addition filters out a meaningful share of false breakouts that a plain crossover would generate.
...
- **Timeframe:** The system is built for higher timeframes; on very low timeframes the envelope expands enough that signals lag badly.
- **Periods:** The 8 and 20 defaults define the system's identity. Shortening or widening them shifts the balance between responsiveness and noise filtering.
- **Envelope multiplier:** Tightening the multiplier produces fewer "confirmed trend" readings, but the ones you do get are more selective. Loosening it produces more signals of lower quality.
...
The cleanest approach is a pullback strategy within the confirmed trend state.
...
The chop state also works as a hard filter — no trades at all when it's active. That alone avoids a lot of ugly ranging sessions.

## Pros & Cons

**Pros:**
- The chop filter reduces whipsaw trades
...
Yes, but crypto's volatility will trigger a tight envelope multiplier too often, so widen it.

**Is it repainting?**
The state indicator and envelope are based on closed candles — no repainting. The MA lines themselves update with each new candle, but that's standard.

## Final Verdict

The 8020_System_Indicator doesn't reinvent the wheel, but it does build a better wheel for a specific job. It won't make you a fortune on its own — no indicator will — but as a trend filter and trade management tool, it's genuinely useful. The chop filter alone is worth the install. The missing star is for the lack of alerts and the fixed volatility calculation that needs manual adjustment across asset classes.
