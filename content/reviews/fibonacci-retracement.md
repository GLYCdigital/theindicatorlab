...
Fibonacci Retracement is the Swiss Army knife of technical analysis — but it is often used poorly. It does not predict the future. What it does is map out potential support and resistance levels based on the Fibonacci sequence: 23.6%, 38.2%, 50%, 61.8%, 78.6%, and 100%.
...
- **Customizable levels**: You can toggle which Fibonacci ratios appear.
- **Extended lines**: The lines extend beyond your drawn range, showing where price might reverse if it keeps trending.
...
- **Levels**: The commonly watched ratios are 38.2%, 50%, and 61.8%. The 50% level is not a true Fibonacci number, but it functions as a psychological magnet.
- **Style**: Dashed lines with some transparency tend to keep the chart readable; solid lines can clutter it.
- **Extensions**: These can be enabled to project targets beyond the drawn range.
- **Color**: Use contrasting colors to separate retracement zones from extension zones.

The general principle when tuning: fewer, higher-conviction levels make for a cleaner chart, and the extension feature is what turns the tool from a static map into something forward-looking.

## How to Use It for Entries and Exits

**Entry logic**:
- In an uptrend, wait for price to pull back to 38.2% or 61.8%. If you see a bullish candlestick pattern (hammer, bullish engulfing) at that level, that is a long setup.
- In a downtrend, wait for a rally to 61.8% or 78.6% and a bearish reversal candle.

**Exit logic**:
- Take partial profits at a retracement of the retracement itself. The extension tool can be used to project a target beyond the initial move.
- Stop loss: place it just beyond the 78.6% level. If price breaks that, the retracement is failing.
...
- Higher probability when combined with confluence.

**Cons**:
- Overused — many traders draw the same levels, leading to fakeouts.
- Subjective. Your swing high might differ from another trader's. This inconsistency hurts reliability.
...
But if you learn to draw Fibonacci manually and master confluence, you don't need alternatives.
