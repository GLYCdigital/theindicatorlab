...
The key feature here is the adjustable "preceding trend" parameter. This isn't a fixed lookback — you can define how many bars must precede the pattern to confirm the setup. On the MACD chart shown above, you can see how the indicator aligns signals with momentum shifts rather than just price action alone. The confluence between the candle pattern and MACD histogram turning positive is cleaner than using either signal independently.

## Settings and How to Tune Them

The indicator exposes a small set of parameters, and the practical question for each is what you're trying to filter out.

- **Preceding trend length** — Controls how many bars of prior bearish or consolidating price action must exist before the pattern qualifies. Shorter settings catch more setups but also more noise in ranging markets; longer settings wait for deeper context and miss earlier reversals. The right value depends on the timeframe you trade and how much chop you're willing to sit through.
- **Body-to-range ratio** — Filters out candles that technically qualify as bullish but have large wicks and small bodies. Raising it demands more conviction per candle; lowering it loosens the definition.
- **Pullback mode** — Lets the pattern trigger during a pullback within an uptrend rather than only after a downtrend. This broadens the setups the indicator will flag, at the cost of some selectivity.

For the MACD chart type specifically, pairing the indicator's signals with the MACD line crossing above the signal line is a common confluence approach. Whether that improves results is something you'd need to verify on your own data — the indicator itself does not enforce it.
...
5. **Target** the prior swing high or a fixed multiple of your risk

What makes this approach appealing is the asymmetry. The stop is tight because the pattern's strength means the first candle's low shouldn't be violated if the reversal is real. The target is usually well-defined by previous structure.
...
- The trend-length filter reduces false signals compared to a bare pattern scanner
- The pattern logic applies across timeframes
...
- Can lag in fast-moving markets — by the time all three candles print, a sharp reversal may already be well underway
...
It's less useful for scalpers on very short timeframes where the pattern is too noisy, and overkill if you already have a robust trend-following system that doesn't use candlestick patterns at all.
...
Yes, but with caveats. The 24/7 market creates more chop, so higher timeframes tend to give cleaner readings and a longer trend-length setting helps filter noise.
...
The Three_White_Soldiers indicator does one thing well: identifying a classic bullish reversal pattern with enough contextual filtering to keep false signals manageable. It won't replace your core strategy, but as a confluence tool or an early-warning system for trend reversals, it's genuinely useful.
...
Three_White_Soldiers delivers solid value for traders who need trend analysis and pattern detection with contextual filtering.
