...
- **No repaint** — Signals stick once the bar closes. You won't get phantom entries.
- **Customizable sensitivity** — The "Sensitivity" input lets you dial in how aggressive the breakout filter is, so you can adapt the trigger to the instrument you're trading.
...
| Setting | Purpose |
|---------|---------|
| Length | Controls the lookback period for the rolling true range average. Shorter values make the indicator more reactive; longer values smooth it out. |
| Multiplier | Sets how far price must move beyond the average range to trigger a signal. Higher values filter for larger breakouts. |
| Show Labels | Toggles on-chart labels for signals. Useful while learning the indicator's behavior. |

**Pro tip:** Pair this with a simple EMA on a higher timeframe. The indicator works best when the signal aligns with the trend direction—don't fight the EMA slope.
...
**Entry logic:**
- Wait for a bullish signal after a pullback to a moving average on the intraday chart.
- Enter on the next bar open with a stop below the signal bar's low.
- Take profit at a multiple of the initial risk. This indicator catches runners, not grinders.
...
**What doesn't work:**
- Don't take every signal. In choppy, range-bound markets, false signals increase. Filter with a volume surge or a higher-timeframe trend.
...
| Clean, fast-loading code | Better suited to futures than stocks/crypto |
...
- **Futures scalpers** who trade ES, NQ, YM, or CL on short intraday charts.
- **Traders who hate repainting indicators** and want something they can evaluate.
...
Honestly, Jonnyfutures_Ib is one of the better no-repaint momentum scanners out there. It's not revolutionary, but it does its job cleanly.
...
A: No. Signals are fixed after the bar closes.

**Q: Can I use it on crypto or forex?**  
A: It works, but false signals increase. The volatility patterns in crypto are erratic enough that the default multiplier may need raising.

**Q: What timeframes are best?**  
A: Short intraday timeframes. Anything higher and the signals are too rare to be useful.
