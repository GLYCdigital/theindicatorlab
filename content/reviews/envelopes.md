...
Envelopes is a straightforward channel indicator. Here's what it does and where it fits.

**What this indicator actually does**
Envelopes draws two bands above and below a moving average (default SMA 20). The bands are set at a fixed percentage offset (default 2.5%). That's it. No adaptive math, no machine learning. Just a channel that expands and contracts with price—but at a constant percentage, not volatility. It's essentially static Bollinger Bands without the dynamic standard deviation.
...
The two inputs are the MA type/length and the deviation percentage. Common configurations include:
- For swing trading on 4H/1D: EMA for faster reaction than SMA, MA length 20, deviation 2.5%.
- For scalping on 5M/15M: SMA for a smoother line with fewer false signals, MA length 10, deviation 1.5%.
...
The nuance—in strong trends, price can ride the upper band for days. Don't short just because it hits the band. Wait for a bearish divergence or a candlestick rejection (e.g., shooting star at upper band).

Bands act as dynamic support/resistance. On the daily, BTC bounced off the lower band twice before breaking higher. Exits: trail stop below the middle MA.
