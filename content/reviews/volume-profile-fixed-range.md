...
- **Fixed range control:** You decide the exact start and end. This is useful for analyzing a specific swing leg.
- **Value Area calculation:** It auto-highlights the Value Area (VA) — the price range where the bulk of volume traded. No manual math.
- **Point of Control (POC):** The single price with the highest volume. The indicator draws a bold horizontal line.
- **Customizable resolution:** You can set tick size or use a basic row count. Tick size gives finer control over row granularity.
- **Volume delta option:** Shows buy vs. sell volume within the range. Useful for spotting absorption or exhaustion.

**Settings and How to Tune Them**

- **Range Type:** "Price" gives you direct control. "Time" anchors the profile to a specific session instead.
- **Value Area %:** Often left at the default, reflecting the conventional Market Profile convention.
- **Number of Rows:** A lower row count gives coarser detail; a higher count gives finer detail but more noise. The right value depends on the instrument and the timeframe you're analyzing.
- **Show Volume Delta:** Optional. If enabled, treat it as confirmation rather than a standalone signal.
- **POC Line Style:** A thicker line is easier to see on a busy chart.

**How to Use It for Entries and Exits**

This isn't a standalone system. It's a context tool. A typical workflow:
...
**Pros:**
- Much cleaner than a rolling volume profile. No recalculation as new bars form.
- The fixed POC can act as a support/resistance reference.
- Lightweight. Doesn't lag even on lower timeframes.
...
This is for intermediate to advanced traders who already use volume profile and want to isolate specific zones. It's suited for:
...
It's **not** for scalpers who need real-time, auto-updating profiles every bar. For that, the built-in Volume Profile on TradingView is the natural fit.
...
A: Yes, but tick size matters — very fine rows will pick up noise on high-priced instruments.
...
**Q: Is it worth paying for from some script authors?**
