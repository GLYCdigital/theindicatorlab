...
---
**Pivot Points Standard** is one of those indicators you install, glance at, and realize you've seen a hundred times. It calculates the classic floor pivot levels — Pivot, R1–R3, S1–S3 — based on the previous period's high, low, and close. No AI, no adaptive lines, no volatility bands. Just raw math.

If you're new to TradingView and want pivot points without clutter, this does the job. If you've been around the block, you'll likely find it basic.
...
Not much. But there are two things worth mentioning:

1. **Multi-timeframe support** – You can set the calculation to daily, weekly, or monthly while viewing a lower timeframe chart. Useful for intraday traders who want weekly levels on a shorter chart.
2. **Clean labeling** – The levels are labeled clearly (R1, Pivot, S1, etc.) and you can toggle visibility per level. Simple, but some pivot indicators clutter the chart with extra lines or annotations.
...
- **Line style & color**: R1–R3 in red tones (dashed for R2, solid for R3) and S1–S3 in green keeps the levels visually grouped. A thicker line for the Pivot separates it from the rest. The point is to prevent visual overload, not to improve the calculation.
...
A common configuration is a daily calculation with all levels shown, R2 and S2 dashed, and S3 and R3 hidden to keep the chart readable. Whether that suits you depends on how often your instrument reaches the outer levels.

## How to use it for entries and exits

Pivot points are reactive, not predictive. Here are the most common ways they're used:

- **Bounce off Pivot**: If price pulls back to the daily pivot and shows a bullish candlestick pattern (hammer, engulfing) with volume, some traders take a long. Stop loss below the pivot. Target R1.
- **Break of R1 with retest**: If price breaks above R1 and then retests it as support, that's a continuation entry. Target R2.
- **S1 as floor**: In a downtrend, if price hits S1 and forms a bullish divergence on RSI, some traders look for a scalp back to the pivot.

**Don't** use them as hard stop-loss levels. They're zones, not exact lines. Price often wicks through S1 before reversing — wait for confirmation.
