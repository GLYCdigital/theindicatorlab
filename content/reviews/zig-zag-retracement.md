...
- **Adaptive Depth**: The deviation percentage adjusts dynamically based on ATR if you enable the "ATR-Based" checkbox. This helps on crypto, where volatility swings wildly.
- **Trend Confirmation Filter**: There's a hidden EMA crossover filter (default 21/50) that only paints retracement zones when the trend aligns. It reduces false signals in chop, but it adds lag.
...
- **Alerts**: Native alert conditions for "Price Entered Zone" and "Zone Rejected" — both work without pine-script hacks.

## Settings and How to Tune Them

The Depth setting is the primary control, and it needs to be adjusted per asset class. A fixed percentage that works on forex will feel too tight on indices and too loose on crypto.

- **Scalping**: Use a shallower Depth, and consider leaving the EMA filter off, since the lag hurts fast moves.
- **Swing Trading**: Use a deeper Depth and turn the EMA filter on to avoid counter-trend retracements.
- **Crypto**: Enable ATR-Based depth so the threshold scales with volatility.

The ATR multiplier and the EMA filter periods are adjustable, but which values suit you depends on the instrument and timeframe you trade. There is no universal best setting.

## How to Trade It

The general setup:

1. Wait for the indicator to paint a fresh retracement zone after a confirmed swing (the first touch is the highest-probability touch).
2. Enter on a close inside the 38.2%–61.8% band, with a stop just beyond the 78.6% level.
3. Take partial profits at the 127.2% extension, and trail the rest with a moving average.
...
- The ATR-adaptive depth addresses the multi-asset problem
...
This indicator suits **pullback traders** who already have a solid entry strategy and just need a reliable trend framework. If you're a trend-follower who likes to buy strength and sell weakness, the retracement zones give you a clear "where not to enter" map.

It's not for mean-reversion traders — the whole logic assumes trend continuation. And if you can't handle repainting, stick with standard pivot-based indicators like the built-in Zig Zag.
...
**What timeframe works best?** Higher intraday timeframes tend to be more stable. On very low timeframes, the ATR-adaptive depth becomes erratic.

**Is it worth paying for?** It's free. The question is whether it's worth your chart space.

## Final Verdict

The Zig_Zag_Retracement does exactly what it promises — nothing more, nothing less. It's not revolutionary, but it's well-executed for a free trend indicator. The ATR-adaptive depth and multi-timeframe pivots are features you'd expect in a paid tool. The repainting is an unavoidable trade-off, but if you respect its limitations and pair it with solid price action, it becomes a genuinely useful piece of your trading stack.
