...
The indicator is not a "set and forget" magic bullet, but it does the heavy lifting of pattern recognition faster than drawing harmonics manually with a ruler and Fib tool.
...
- **PRZ box** – The shaded Potential Reversal Zone shows where D completes, so you can manually check confluence (support/resistance, RSI divergence).
- **Alerts** – You can set alerts for "New Pattern Confirmed" or "Pattern Completed."
...
Open the settings (gear icon). The available controls are:

- **Patterns to scan** – Choose which harmonic patterns the scanner looks for.
- **Max lookback bars** – Controls how much historical data the scanner uses to find the initial leg (XA).
- **Min pattern size** – Sets the smallest pattern the scanner will draw. Too small and you get noise on very low timeframes.
- **Show PRZ** – Toggles the Potential Reversal Zone box.
- **Show Fib levels** – Toggles the Fibonacci levels, with a transparency control.
- **Alert on completion** – Fires an alert when a pattern completes at point D.

For lower timeframes, tightening the pattern size filter helps avoid multi-day swings being drawn on intraday charts; on higher timeframes you can loosen it.

**How to use it for entries and exits**

Not every pattern it draws is worth trading. A workable filter:
...
3. **Entry** – On a bullish pattern, a limit order at the 0.786 or 0.886 Fib retracement (common D completion points). Stop loss below X (for bullish) or above X (for bearish).
4. **Exit** – Take profit at the 0.618 or 1.272 Fib extension of the AD leg. The indicator doesn't draw TP levels, so you add a manual Fib tool.

**Honest pros and cons**

**Pros**
- Saves time. Pattern drawing is tedious.
- The PRZ box is a useful reference point for manual confluence checks.
- Free version is fully functional (no paywall for core features).
- Works on crypto, forex, and indices.

**Cons**
- False positives on low timeframes (1m–5m). Avoid those.
...
This is for intermediate to advanced traders who already understand harmonic patterns. If you don't know what a Gartley or Bat is, you'll get confused by the labels. Beginners should learn the patterns manually first. Also, it's not for scalpers—patterns need time to develop.

**Better alternatives if they exist**

- **Harmonic Patterns 4 in 1** by LuxAlgo – More advanced, with TP/SL zones and trend filtering. But it's paid.
...
Yes. Works best on 4H and above. On 1H, you get smaller patterns with higher noise.
...
Yes. An RSI divergence filter works well—divergence at the D point is a useful confluence check.
