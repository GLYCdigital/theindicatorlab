...
The remaining modules round things out: **ORB** for opening range breakouts with a user-defined session and UTC offset, one breakout or breakdown signal per session on a close through the range; **TrendLines** drawing time-linear wedge trendlines from consecutive pivots, extended 75 bars past the confirming pivot, with preset pivot lengths from Small (10/5) to Macro (30/15); **RMI Trend**, a momentum ribbon averaging RSI and MFI that flips colour on threshold crosses; **Capital Risk**, a position-sizing table taking capital, risk per trade, target RR and stop-loss percentage; and **Volume Screener**, a table of daily dollar volume (close × volume) for up to 40 symbols, tinted green or red against a "Trading Capital" threshold in millions.
...
Discretionary traders who already have a process and want volume context layered onto structure — particularly those trading liquidity zones or order blocks manually. It's less suited to anyone wanting a single mechanical signal, and anyone who won't invest time learning the module interactions will find it noisy.

## FAQ

**Does it repaint?** The trend module uses ATR trailing bands with flip labels — these confirm on close. Pivot-based modules necessarily need the right-side bars to confirm. Plan accordingly.
