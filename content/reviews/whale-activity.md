...
The indicator pulls from aggregated exchange data (Binance, Coinbase, Kraken) and filters for trades exceeding a configurable multiple of the average trade size.
...
- **Customizable sensitivity** – You can set the "whale threshold" as a multiplier of average trade size. Lower it for smaller accounts, or raise it to isolate true institutional moves.
...
- **Threshold multiplier** – Sets how far above average trade size a transaction must be to count as a whale move. Lower values catch more activity; higher values isolate larger players.
- **Minimum cluster volume** – A floor on total volume before a cluster is highlighted, so a single oversized print doesn't get flagged as accumulation or distribution.
- **Lookback period** – Controls the window used to establish the average trade size. Longer lookbacks smooth out noise but introduce lag.
- **Timeframe display** – The indicator can be shown across timeframes, but lower timeframes tend to get flooded with activity from arbitrage bots.

## How to Use It for Entries and Exits

**Entry:** Look for a green cluster forming near a key support level. Wait for price to break above the cluster's high with volume confirmation.

**Exit:** Red clusters near resistance are your warning. If you see several red clusters in a short window, tighten stops. The indicator won't tell you the exact top, but it's screaming "distribution" when combined with RSI divergence.
...
- Useless on low-liquidity pairs – nothing but noise on micro-cap altcoins.
...
Yes, but only if futures volume data is available via the exchange.
...
Higher timeframes give the cleanest signals. Daily is too slow; lower intraday timeframes are noisy.
